...
public class MolybdeniteBlock extends ChemcraftModElements.ModElement {
	@ObjectHolder("chemcraft:molybdenite")
	public static final Block block = null;
	public MolybdeniteBlock(ChemcraftModElements instance) {
		super(instance, 275);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("molybdenite", "molybdenite", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 9)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(6, 0, 0, 128))));
		}
	}
}
