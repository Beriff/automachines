...
import net.beriff.chemistrycraft.item.SphaleriteDustItem;
...
public class SphaleriteBlock extends ChemcraftModElements.ModElement {
	@ObjectHolder("chemcraft:sphalerite")
	public static final Block block = null;
	public SphaleriteBlock(ChemcraftModElements instance) {
		super(instance, 230);
	}
	...
	public static class CustomBlock extends Block {
		...
		public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
			...
			return Collections.singletonList(new ItemStack(SphaleriteDustItem.block, (int) (1)));
		}
	}
	@Override
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("sphalerite", "sphalerite", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 11)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(6, 0, 0, 64))));
		}
	}
}
