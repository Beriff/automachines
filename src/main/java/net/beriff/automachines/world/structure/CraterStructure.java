...
package net.beriff.automachines.world.structure;
...
public class CraterStructure extends AutomachinesModElements.ModElement {
	public CraterStructure(AutomachinesModElements instance) {
		super(instance, 57);
	}
	...
	public void init(FMLCommonSetupEvent event) {
				...
				if ((random.nextInt(1000000) + 1) <= 20000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("automachines", "crater2"));
						if (template == null)
							return false;
		...
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			if (ForgeRegistries.BIOMES.getKey(biome).equals(new ResourceLocation("automachines:volcano_lands")))
				biomeCriteria = true;
			if (!biomeCriteria)
				continue;
