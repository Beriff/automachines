...
public class PebbleSpawnStructure extends AutomachinesModElements.ModElement {
	public PebbleSpawnStructure(AutomachinesModElements instance) {
		super(instance, 68);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 50000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("automachines", "pebble"));
						if (template == null)
							return false;
