...
package net.beriff.automachines.world.biome;
...
import net.beriff.automachines.block.AshBlockBlock;
...
public class VolcanoLandsBiome extends AutomachinesModElements.ModElement {
	@ObjectHolder("automachines:volcano_lands")
	public static final CustomBiome biome = null;
	public VolcanoLandsBiome(AutomachinesModElements instance) {
		super(instance, 12);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.3f).depth(0.7999999999999999f).scale(0.2f).temperature(1.7000000000000002f)
					.precipitation(Biome.RainType.RAIN).category(Biome.Category.MESA).waterColor(-15321476).waterFogColor(-15321476).parent("desert")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(AshBlockBlock.block.getDefaultState(),
							AshBlockBlock.block.getDefaultState(), AshBlockBlock.block.getDefaultState())));
			setRegistryName("volcano_lands");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							...
							.withPlacement(Placement.COUNT_EXTRA_HEIGHTMAP.configure(new AtSurfaceWithExtraConfig(11, 0.1F, 1))));
		}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == AshBlockBlock.block.getDefaultState().getBlock() || ground == AshBlockBlock.block.getDefaultState().getBlock())
							&& (ground2 == AshBlockBlock.block.getDefaultState().getBlock()
									|| ground2 == AshBlockBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), AshBlockBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.OBSIDIAN.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == AshBlockBlock.block.getDefaultState().getBlock()
					|| blockType == AshBlockBlock.block.getDefaultState().getBlock();
		}
